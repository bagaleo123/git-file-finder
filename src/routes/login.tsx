...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Sign In — The Labor Shield Thailand" },
      { name: "description", content: "Sign in to your private case room. Save your case, upload documents, and get personalised AI legal guidance." },
    ],
  }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  async function googleLogin() {
    ...
    if (r.error) { setLoading(false); toast.error("Google sign-in failed"); }
  }
  ...
  return (
    <div className="min-h-screen grid place-items-center px-4 py-10">
      <div className="w-full max-w-md glass rounded-2xl p-8">
        ...
        <h1 className="text-2xl font-bold">Sign in to your case room</h1>
        <p className="text-sm text-muted-foreground mt-1">Save cases, upload documents, get personalised AI guidance.</p>

        <button onClick={googleLogin} disabled={loading} className="mt-6 w-full h-11 rounded-xl bg-surface border border-border/60 hover:border-primary/60 transition flex items-center justify-center gap-2 text-sm font-medium">
        ...
        <form onSubmit={emailLogin} className="space-y-3">
          ...
          <label className="block">
            ...
            <div className="mt-1 flex items-center gap-2 bg-surface/60 border border-border/60 rounded-xl px-3">
              ...
              <input required type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="flex-1 bg-transparent py-3 text-sm outline-none" placeholder="••••••••" />
            </div>
          </label>
        ...
        <p className="text-center text-sm text-muted-foreground mt-5">No account? <Link to="/signup" className="text-primary font-medium">Create one</Link></p>
      </div>
    </div>
  );
}
